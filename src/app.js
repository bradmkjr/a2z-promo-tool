...
app.get('/follow', function(req, res) {
  ...
  var T = new Twit( {
    ...
    var randomTag = tags[getRandomRange(0,tags.length)];

  	//
	//  search twitter for all tweets containing the word 'banana' since July 11, 2011
	//
	T.get('search/tweets', { q: randomTag, count: 5 }, function(error, data, response) {
	  // console.log(data);
	  // console.log(response);
	  ...
	  if( data != undefined && !error && response.statusCode == 200 ){
	  	...
	  	// console.log(status.user.id_str);
	  	// res.write( status.user.id_str );	
	  	// res.write( status.user.name );	
	  	console.log( status.user.screen_name );	
	  	res.write( status.user.screen_name );	
...
app.get('/ghost', function(req, res) {
	...
	var url = 'https://affiliate-program.amazon.com/';	
...
app.get('/login', function(req, res) {
	...
	request({url: url, jar: j}, function (error, response, body) {
		...
		if (!error && response.statusCode == 200) {
			...
			url = 'https://affiliate-program.amazon.com/login';
			...
			request({	url: url, 
						...
						// res.end();
							...
							form['email'] = process.env.amazon_email;

							form['password'] = process.env.amazon_password;
							...
							// url = 'https://requestb.in/swma0wsw';
							...
							var options = {	url: url, 
											...
											form: {
												...
												appActionToken: form['appActionToken'],
												appAction: form['appAction'],
												'openid.pape.max_auth_age': form['openid.pape.max_auth_age'],
												'openid.return_to': form['openid.return_to'],
												prevRID: form['prevRID'],
												'openid.identity': form['openid.identity'],
												'openid.assoc_handle': form['openid.assoc_handle'],
												'openid.mode': form['openid.mode'],
												prepopulatedLoginId: form['prepopulatedLoginId'],
												marketPlaceId: form['marketPlaceId'],
												failedSignInCount: form['failedSignInCount'],
												'openid.claimed_id': form['openid.claimed_id'],
												pageId: form['pageId'],
												'openid.ns': form['openid.ns'],
												create: form['create'],
												rememberMe: form['rememberMe'],
												ue_back: form['ue_back']
											}
				...
				} // end !error && response.statusCode == 200
				else
				{
					console.log(error);
					// res.write(error);
...
app.get('/promo', function(req, res) {
  ...
  function loadHTML(){
  	...
  	cache.getCache(promoCodesURL, function(err,data){
        if(err || data == undefined ){
        	// cache MISS
        	console.log('cache MISS')
        	//res.write('cache MISS');
        	request(promoCodesURL, function(error, response, data){

        		// console.log(data);

				// First we'll check to make sure no errors occurred when making the request
		        if(!error){		       
					cache.setCache( promoCodesURL, data, function(){
						processPromos(data);
					});
		        }
		    });     
